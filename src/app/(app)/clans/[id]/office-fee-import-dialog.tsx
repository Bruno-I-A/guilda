          ...
          {!preview ? (
            ...
            <div className="grid gap-4">
              <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                <div className="rounded-lg bg-emerald-500/10 p-3 text-center"><strong className="block font-mono text-xl text-emerald-400">{summary.createdProfiles}</strong><span className="text-xs">cadastros criados</span></div>
              ...
              <div className="grid max-h-[48vh] gap-2 overflow-y-auto pr-1">
                {preview.rows.map((row) => {
                  ...
                  return <article key={row.id} className="grid gap-2 rounded-lg border bg-card/50 p-3 sm:grid-cols-[minmax(12rem,1fr)_minmax(14rem,1fr)_auto] sm:items-center">
                    ...
                    <div className="min-w-0">{resolution?.ignored ? <Badge variant="outline">Linha ignorada</Badge> : selectedName ? <p className="flex items-center gap-1.5 text-sm"><Check className="size-4 text-emerald-400" aria-hidden /><span className="truncate">{selectedName}</span>{selectedClient && !selectedClient.active ? <Badge variant="outline">inativa</Badge> : null}{row.status !== "matched" ? <Badge variant="outline">confirmada manualmente</Badge> : null}</p> : <p className="flex items-center gap-1.5 text-sm text-destructive"><AlertTriangle className="size-4" aria-hidden /> Sem conciliação</p>}<p className="line-clamp-2 text-[11px] text-muted-foreground">{row.explanation}</p>{row.issues.length > 0 ? <p className="text-[11px] text-amber-300">{row.issues.map((issue) => `${COLUMN_LABELS[issue.field] ?? issue.field}: ${issue.message}`).join(" ")}</p> : null}{selectedClient ? <details className="mt-1 text-[11px]"><summary className="cursor-pointer text-muted-foreground">{changes.length > 0 ? `${changes.length} alteração(ões) no cadastro` : "Nenhuma alteração no cadastro"}</summary>{changes.length > 0 ? <ul className="mt-1 grid gap-1 rounded border bg-muted/30 p-2">{changes.map((change) => <li key={change.field}><strong>{change.field}:</strong> <span className="text-muted-foreground">{change.before}</span><span aria-hidden> → </span>{change.after}</li>)}</ul> : null}</details> : null}</div>
